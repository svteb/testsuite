...
task "smf_upf_heartbeat" do |t, args|
  CNFManager::Task.task_runner(args, task: t) do |args, config|
    ...
    if smf && upf 
      ...
      capture.begin_capture_by_label(smf_key, smf_value, command)

      unless baseline_count
        sleep 5
        baseline_count = capture.regex_search(/"pfcp\.msg_type": "(1|2)"/).size
      ...
      chaos_count = capture.regex_search(/"pfcp\.msg_type": "(1|2)"/).size
...
task "suci_enabled" do |t, args|
  CNFManager::Task.task_runner(args, task: t) do |args, config|
    ...
    if core 
      ...
      capture = K8sTshark::TsharkPacketCapture.new
      capture.begin_capture_by_label(core_key, core_value, command)
